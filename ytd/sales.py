...
def get_remaining_days():
    ...
    return remaining_start.strftime("%Y-%m-%dT00:00:00Z"), remaining_end.strftime("%Y-%m-%dT23:59:59Z")
...
def get_remaining_days_last_year():
    ...
    try:
        ...
        # Calculate the day to use, capped to the last valid day of the month
...
os.makedirs('ytdData', exist_ok=True)

# Fetch report definitions for YTD and LYTD sales data
def get_report_definitions():
    ...
    print(f"Remaining days start: {remaining_days_start}, end: {remaining_days_end}")
    ...
    reports = [
        ...
        # Last year's full months
        {
            ...
            "report_name": "lytd_last_year_full_months"
        }
    ]

    # For the remaining days this year, split into smaller reports if necessary
    remaining_days = datetime.strptime(remaining_days_start, "%Y-%m-%dT%H:%M:%SZ"), datetime.strptime(remaining_days_end, "%Y-%m-%dT%H:%M:%SZ")
    if (remaining_days[1] - remaining_days[0]).days > 14:
        chunks = split_date_range(remaining_days[0], remaining_days[1])
        ...
        reports.append({
            "report_type": "GET_VENDOR_SALES_REPORT",
            "start_time": remaining_days_start,
            "end_time": remaining_days_end,
            "fieldnames": ["startDate", "endDate", "asin", "shippedCogs", "shippedRevenue", "shippedUnits"],
            "report_options": {
                "reportPeriod": "DAY",
                "distributorView": "SOURCING",
    ...
    lytd_remaining_days = datetime.strptime(lytd_remaining_start, "%Y-%m-%dT%H:%M:%SZ"), datetime.strptime(lytd_remaining_end, "%Y-%m-%dT%H:%M:%SZ")
    if (lytd_remaining_days[1] - lytd_remaining_days[0]).days > 14:
        chunks = split_date_range(lytd_remaining_days[0], lytd_remaining_days[1])
        for idx, (start, end) in enumerate(chunks):
            reports.append({
                "report_type": "GET_VENDOR_SALES_REPORT",
                "start_time": start.strftime("%Y-%m-%dT00:00:00Z"),
    ...
    for _ in range(retries):
        try:
            ...
            if status["processingStatus"] == "DONE":
                ...
                asin_csv_filename = f"ytdData/{report_name}_asin_report.csv"
...
def combine_ytd_reports():
    # Define the folder path for YTD sales reports
    ytd_folder = 'ytdData'
    
    # Find all parts of this year's reports dynamically, including `ytd_this_year_asin_report.csv`
    this_year_files = glob.glob(f"{ytd_folder}/ytd_this_year_*_asin_report.csv") + [f"{ytd_folder}/ytd_this_year_asin_report.csv"]
    ...
    if this_year_files:
        ...
        if this_year_dfs:
            ...
            combined_this_year_df.to_csv(f"{ytd_folder}/combined_ytd_this_year.csv", index=False)
            print("Combined YTD sales data for this year saved.")
        else:
            print("No valid files found for this year's YTD sales data.")
    else:
        print("No files for this year's YTD sales data found.")
    
    # Find all parts of last year's reports dynamically, including `ytd_last_year_asin_report.csv`
    last_year_files = glob.glob(f"{ytd_folder}/lytd_*_asin_report.csv") + [f"{ytd_folder}/ytd_last_year_asin_report.csv"]
    ...
    if last_year_files:
        ...
        if last_year_dfs:
            ...
            combined_last_year_df.to_csv(f"{ytd_folder}/combined_ytd_last_year.csv", index=False)
            print("Combined YTD sales data for last year saved.")
        else:
            print("No valid files found for last year's YTD sales data.")
    else:
        print("No files for last year's YTD sales data found.")
